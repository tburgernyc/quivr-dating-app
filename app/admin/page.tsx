...
function Kpi({ title, type, suffix=''}:{ title:string; type:string; suffix?:string }){
  const [v,setV] = useState<number>(0)
  useEffect(()=>{ (async()=>{ const r=await fetch('/api/admin/metrics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type})}); const d=await r.json(); if(typeof d?.value==='number') setV(d.value) })() },[type])
  return (
    <article className="rounded-2xl shadow p-4">
      <div className="text-xs text-neutral-500">{title}</div>
      <div className="text-2xl font-bold mt-1">{v}{suffix}</div>
    </article>
  )
}
...
function CityHeatmap(){
  ...
  useEffect(()=>{ (async()=>{ const r=await fetch('/api/admin/metrics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'heatmap_city'})}); const d=await r.json(); if(Array.isArray(d?.value)) setRows(d.value) })() },[])
  return (
      ...
      <ul className="mt-3 space-y-1 max-h-80 overflow-auto">
        {rows.map(r=> (
          <li key={r.city} className="flex items-center gap-2 text-sm">
            <span className="w-28 truncate" title={r.city}>{r.city}</span>
            <div className="flex-1 h-2 bg-neutral-200 rounded">
              <div className="h-2 bg-black rounded" style={{ width: `${Math.min(100, r.count*4)}%` }} />
            </div>
...
function TopFavorited(){
  ...
  useEffect(()=>{ (async()=>{ const r=await fetch('/api/admin/metrics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'top_favorited_users_7d'})}); const d=await r.json(); if(Array.isArray(d?.value)) setRows(d.value) })() },[])
  return (
    <article className="rounded-2xl shadow p-4 lg:col-span-1">
...
function ReportReasons(){
  ...
  useEffect(()=>{ (async()=>{ const r=await fetch('/api/admin/metrics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'top_report_reasons_7d'})}); const d=await r.json(); if(Array.isArray(d?.value)) setRows(d.value) })() },[])
  return (
    <article className="rounded-2xl shadow p-4 lg:col-span-1">
...
function Timeseries({ title, type }:{ title:string; type:string }){
  ...
  useEffect(()=>{ (async()=>{ const r=await fetch('/api/admin/metrics',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type})}); const d=await r.json(); if(Array.isArray(d?.value)) setPoints(d.value) })() },[type])
  ...
  return (
    <article className="rounded-2xl shadow p-4 lg:col-span-1">
      ...
      <div className="mt-3 grid grid-cols-7 gap-2 items-end h-32">
        {points.map(p=> (
          <div key={p.date} title={`${p.date}: ${p.count}`} className="bg-black rounded" style={{ height: `${(p.count/max)*100}%` }} />
        ))}
      </div>
