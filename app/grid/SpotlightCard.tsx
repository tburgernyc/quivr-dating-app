...
export default function SpotlightCard({ user }: Props){
  ...
  return (
    <article
      ...
      onKeyDown={(e)=>{ if(e.key==='Enter' || e.key===' ') window.location.href=`/profile/${user.id}` }}
    ...
    >
      <div className="flex items-center gap-3">
        <div className="relative w-12 h-12 rounded-full overflow-hidden ring-1 ring-black/5 bg-gradient-to-br from-neutral-200 to-neutral-100 flex items-center justify-center text-sm font-semibold">
          {avatar ? <img src={avatar} alt="" className="w-full h-full object-cover" /> : <span>{initials(name)}</span>}
        </div>
        <div className="min-w-0">
          <div className="text-sm font-semibold truncate">{name}</div>
          <div className="text-xs text-neutral-600 truncate">{age}{city}</div>
        </div>
      </div>
      <button
